...
//      "December"};
  ...
  private static final String[] situacionesJuego = new String[] {
      "Gol", "Falta Z1", "Falta Z2", "Falta Z3", "Recuperación Z1", "Recuperación Z2", "Recuperación Z3","Tiro Libre Z1","Tiro Libre Z2","Tiro Libre Z3", "Centrao Lateral",
      "Entradas Erradas", "Fuera Lugar", "Opción Gol", "Penalty","Remate","Tiro Esquina", "Tarjeta Roja","Tarjeta Amarilla"};

  private static int autoId = 0;
  
  
   public static List<Data> getSituacionesCompetencia(int size, double min, double scale) {
  ...
  public static List<Data> getDataNegative(int size, double min, double scale) {
    List<Data> data = new ArrayList<Data>();
    for (int i = 0; i < size; i++) {
      data.add(new Data(i+"",situacionesJuego[i % size], Math.floor(Math.min(Math.random() * scale, min)), Math.floor(Math.min(Math.random() * scale, min)),
