 ...
 * @author Malejandro
 */
public class ComboBoxTorneo extends ComboBox<BeanModel>{
    ...
    public ComboBoxTorneo(final int tipo) {
        
        this.tipo = tipo;
        usuarioLogueado = BeansLocales.getUsuario();
        
       final RPCAdminTorneosAsync svc = (RPCAdminTorneosAsync) GWT.create(RPCAdminTorneos.class);
        ServiceDefTarget endpoint = (ServiceDefTarget) svc;
        endpoint.setServiceEntryPoint("services/RPCAdminTorneos");
        
        // proxy and reader  
        RpcProxy<List<Torneos>> proxy = new RpcProxy<List<Torneos>>() {

            @Override
            public void load(Object loadConfig, AsyncCallback<List<Torneos>> callback) {
        ...
        setEmptyText("Seleccione un torneo...");
    ...
    public void seleccionar(Long id){
        ...
        for (BeanModel beanModel : lista) {
            Torneos entidad = (Torneos)beanModel.getBean();
     ...
     * @return 
     */
    public Torneos getTorneosElegido() {
        if (getSelection().size() > 0) {
            BeanModel beanModel = ((BeanModel) getSelection().get(0));
            return (Torneos) beanModel.getBean();
        }else{
            return null;
        }
    }
