 ...
 * @author anfeh_000
 */
public class ComboBoxInsEducativa extends ComboBox<BeanModel>{
    ...
    public ComboBoxInsEducativa(final int tipo) {
        this.tipo = tipo;
        usuarioLogueado = BeansLocales.getUsuario();
        
       final RPCAdminInstEducativaAsync svc = (RPCAdminInstEducativaAsync) GWT.create(RPCAdminInstEducativa.class);
        ServiceDefTarget endpoint = (ServiceDefTarget) svc;
        endpoint.setServiceEntryPoint("services/RPCAdminInstEducativa");
        
        // proxy and reader  
        RpcProxy<List<InstEducativa>> proxy = new RpcProxy<List<InstEducativa>>() {

            @Override
            public void load(Object loadConfig, AsyncCallback<List<InstEducativa>> callback) {
        ...
        setEmptyText("Seleccione una Inst. Educativa...");
        setDisplayField("nombreInstEducativa");
        setWidth(150);
    ...
    public void seleccionar(Long id){
        ...
        for (BeanModel beanModel : lista) {
            InstEducativa entidad = (InstEducativa)beanModel.getBean();
     ...
     * @return 
     */
    public InstEducativa getInstEducativaElegida() {
        if (getSelection().size() > 0) {
            BeanModel beanModel = ((BeanModel) getSelection().get(0));
            return (InstEducativa) beanModel.getBean();
        }else{
            return null;
        }
    }
