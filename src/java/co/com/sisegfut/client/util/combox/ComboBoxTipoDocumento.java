 ...
 * @author anfeh_000
 */
public class ComboBoxTipoDocumento extends ComboBox<BeanModel>{
    ...
    public ComboBoxTipoDocumento(final int tipo) {
        this.tipo = tipo;
        usuarioLogueado = BeansLocales.getUsuario();
        
       final RPCAdminTipoDocumentoAsync svc = (RPCAdminTipoDocumentoAsync) GWT.create(RPCAdminTipoDocumento.class);
        ServiceDefTarget endpoint = (ServiceDefTarget) svc;
        endpoint.setServiceEntryPoint("services/RPCAdminTipoDocumento");
        
        // proxy and reader  
        RpcProxy<List<TipoDocumento>> proxy = new RpcProxy<List<TipoDocumento>>() {

            @Override
            public void load(Object loadConfig, AsyncCallback<List<TipoDocumento>> callback) {
        ...
        setEmptyText("Seleccione un tipo de documento...");
        setDisplayField("nombreTipoDocumento");
        setWidth(150);
    ...
    public void seleccionar(Long id){
        ...
        for (BeanModel beanModel : lista) {
            TipoDocumento entidad = (TipoDocumento)beanModel.getBean();
     ...
     * @return 
     */
    public TipoDocumento getTipoDocumentoElegido() {
        if (getSelection().size() > 0) {
            BeanModel beanModel = ((BeanModel) getSelection().get(0));
            return (TipoDocumento) beanModel.getBean();
        }else{
            return null;
        }
    }
