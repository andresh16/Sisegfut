 ...
 * @author anfeh_000
 */
public class ComboBoxPosiciones extends ComboBox<BeanModel> {
    ...
    public ComboBoxPosiciones(final int tipo) {

        this.tipo = tipo;
        usuarioLogueado = BeansLocales.getUsuario();

        final RPCAdminPosicionesAsync svc = (RPCAdminPosicionesAsync) GWT.create(RPCAdminPosiciones.class);
        ServiceDefTarget endpoint = (ServiceDefTarget) svc;
        endpoint.setServiceEntryPoint("services/RPCAdminPosiciones");

        // proxy and reader  
        RpcProxy<List<Posiciones>> proxy = new RpcProxy<List<Posiciones>>() {

            @Override
            public void load(Object loadConfig, AsyncCallback<List<Posiciones>> callback) {
        ...
        setEmptyText("Seleccione una posición...");
        setDisplayField("nombrePosicion");
        setWidth(150);
    ...
    public void seleccionar(Long id) {
        ...
        for (BeanModel beanModel : lista) {
            Posiciones entidad = (Posiciones) beanModel.getBean();
     ...
     * @return
     */
    public Posiciones getPosicionesElegido() {
        if (getSelection().size() > 0) {
            BeanModel beanModel = ((BeanModel) getSelection().get(0));
            return (Posiciones) beanModel.getBean();
        } else {
            return null;
        }
    }
