 ...
 * @author anfeh_000
 */
public class ComboBoxCargos extends ComboBox<BeanModel>{
    ...
    public ComboBoxCargos(final int tipo) {
   this.tipo = tipo;
        usuarioLogueado = BeansLocales.getUsuario();
        
       final RPCAdminCargosAsync svc = (RPCAdminCargosAsync) GWT.create(RPCAdminCargos.class);
        ServiceDefTarget endpoint = (ServiceDefTarget) svc;
        endpoint.setServiceEntryPoint("services/RPCAdminCargos");
        
        // proxy and reader  
        RpcProxy<List<Cargos>> proxy = new RpcProxy<List<Cargos>>() {

            @Override
            public void load(Object loadConfig, AsyncCallback<List<Cargos>> callback) {
        ...
        setEmptyText("Seleccione un cargo...");
        setDisplayField("nombrecargo");
        setWidth(150);
    ...
    public void seleccionar(Long id){
        ...
        for (BeanModel beanModel : lista) {
            Cargos entidad = (Cargos)beanModel.getBean();
     ...
     * @return 
     */
    public Cargos getCargoElegido() {
        if (getSelection().size() > 0) {
            BeanModel beanModel = ((BeanModel) getSelection().get(0));
            return (Cargos) beanModel.getBean();
        }else{
            return null;
        }
    }
